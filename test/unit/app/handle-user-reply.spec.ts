...
describe("handle user reply", () => {
  it("categorizes expense when confidence is above threshold", async () => {
    ...
    const handleUserReply = createHandleUserReply({
      ...
      expenseRepo: {
        createPending: vi.fn(),
        getById: vi.fn().mockResolvedValue({ id: "exp_1" }),
        markCategorized,
      },
      categoryRepo: {
      ...
      conversationState: {
        put: vi.fn(),
        get: vi.fn().mockResolvedValue({ channel: "whatsapp", userId: "u1", expenseId: "exp_1", createdAt: "now" }),
        delete: deleteState,
      },
      logger: {
        debug: vi.fn(),
    ...
    expect(markCategorized).toHaveBeenCalledWith({ id: "exp_1", categoryId: "cat_food" });
    expect(deleteState).toHaveBeenCalledWith({ channel: "whatsapp", userId: "u1" });
