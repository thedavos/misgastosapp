...
import { createOpenRouterAdapter } from "@/adapters/ai/openrouter.adapter";
import { createUserScopedAiAdapter } from "@/adapters/ai/user-scoped-ai.adapter";
import { createVercelAiAdapter } from "@/adapters/ai/vercel-ai.adapter";
...
import { createD1UserAiGatewayRepo } from "@/adapters/persistence/d1/user-ai-gateway.repo";
...
export function createContainer(
  ...
  const platformAi =
    env.AI_PROVIDER === "vercel"
      ? createVercelAiAdapter(env)
      : env.AI_PROVIDER === "openrouter"
        ? createOpenRouterAdapter(env)
        : createCloudflareAiAdapter(env);
  const userAiGatewayRepo = createD1UserAiGatewayRepo(env);
  const ai = createUserScopedAiAdapter({
    env,
    default: platformAi,
    userGatewayRepo: userAiGatewayRepo,
    logger,
  });
