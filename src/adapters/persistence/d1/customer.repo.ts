...
export function createD1CustomerRepo(env: WorkerEnv): CustomerRepoPort {
  return {
    ...
    async createChannelMapping(input: {
      ...
      const userSourceRow = await env.DB
        .prepare(
          `SELECT id, user_id AS customer_id, source_type AS channel, external_id AS external_user_id, is_primary
           FROM user_sources
      ...
      if (userSourceRow) {
        return mapChannel(userSourceRow);
      }
